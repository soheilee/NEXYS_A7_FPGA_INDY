...
(
    ...
    input[DW-1:0]              axis_in_df_tdata,
    input                      axis_in_df_tvalid,
    input                      axis_in_df_tlast,
    output                     axis_in_df_tready,

    // The input stream meta data
    input[DW-1:0]              axis_in_meta_tdata,
    input                      axis_in_meta_tvalid,
    input                      axis_in_meta_tlast,
...
assign axis_in_df_tready = (resetn == 1);
...
always @* begin
    ...
    case (fsm_state)
        PINGPONG_DATAFRAME: begin
                                axis_out_tlast = 0;
                                if (axis_in_df_tvalid) begin
                                    axis_out_tdata = axis_in_df_tdata;
                                    axis_out_tvalid = axis_in_df_tvalid;
                                    axis_out_tlast = axis_in_df_tlast;
                                end
                            end
        ...
        META_DATA:          begin
                                if (axis_in_meta_tvalid) begin
                                    ...
                                    axis_out_tlast = (counter_md == META_DATA_LENGTH);
                                end
                            end
